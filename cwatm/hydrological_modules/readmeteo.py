        ...
        if not self.var.meteomapsscale:
            ...
            if not(check_clim):
               # for downscaling it is always cut from the global map
                if (latldd != latmeteo) or (lonldd != lonmeteo):
                    dlon = cutmapGlobal[1] - cutmapGlobal[0]
                    dlat = cutmapGlobal[3] - cutmapGlobal[2]
                    cutmapGlobal[0] = int(cutmap[0] / maskmapAttr['reso_mask_meteo'])
                    cutmapGlobal[2] = int(cutmap[2] / maskmapAttr['reso_mask_meteo'])
                    cutmapGlobal[1] = cutmapGlobal[0] + dlon
                    cutmapGlobal[3] = cutmapGlobal[2] + dlat
...
# --------------------------------------------------------------------------
    ...
    def dynamic(self):
        ...
        self.var.Precipitation = self.var.Precipitation * self.model.DtDay * self.var.con_precipitation
