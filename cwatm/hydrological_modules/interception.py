    ...
    **Functions**
    ...
    def initial(self):
        ...
        for coverNum, coverType in enumerate(self.model.coverTypes):
            ...
            self.var.minInterceptCap[coverType_indices] = self.model.to_subvar(data=loadmap(coverType + "_minInterceptCap"), fn=None)
            if coverType in ('forest', 'grassland', 'irrPaddy', 'irrNonPaddy', 'sealed'):
                initial = self.model.to_subvar(data=self.var.load_initial(coverType + "_interceptStor"), fn=None)
    ...
    def dynamic(self, potTranspiration):
        ...
        for coverNum, coverType in enumerate(self.model.coverTypes):
            ...
            if coverType in ('forest', 'grassland'):
                ...
                covertype_interceptCapNC = self.model.to_subvar(data=covertype_interceptCapNC, fn=None)  # checked
