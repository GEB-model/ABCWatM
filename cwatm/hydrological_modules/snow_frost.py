    ...
    **Functions**
    ...
    def initial(self, ElevationStD):
        ...
        * loads the parameter for frost
        ...
        self.var.DeltaTSnow = ElevationStD * self.model.to_subvar(data=loadmap('TemperatureLapseRate'), fn=None)  # checked
        ...
        for i in range(self.var.numberSnowLayers):
            self.var.SnowCoverS[i] = self.model.to_subvar(data=self.var.load_initial("SnowCover", number=i+1, default=self.model.var.full_compressed(0, dtype=np.float32)), fn=None)
        ...
        self.var.FrostIndex = self.model.to_subvar(data=self.var.load_initial('FrostIndex'), fn=None)
