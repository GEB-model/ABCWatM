        ...
        if checkOption("calc_environflow"):
            ...
            if returnBool("calc_ef_afterRun"):
                ...
                try:
                    ...
                    msg = (
                        'OUT_MAP_Daily = discharge may be not defined in [OUTPUT] \n in the settings file: "'
                        + ssettingsfile[0]
                        + '"\n'
                    )
    ...
    def dynamic(self):
        ...
        if checkOption("calc_environflow"):
            if (returnBool("calc_ef_afterRun") == False) or (
                ...
                datearray = np.arange(
                    dateVar["dateStart"], dateVar["dateEnd"], datetime.timedelta(days=1)
                ...
                for key in meteofiles[name]:
                    ...
                    value = list(nf1.variables.items())[-1][
                        0
                    ...
                    for i in range(monthshape):
                        if returnBool("cut_ef_map"):
                            mapnp = nf1.variables[value][
                                i, cutmap[2] : cutmap[3], cutmap[0] : cutmap[1]
                            ]
                        else:
                ...
                for i in range(12):
                    self.var.EF_VMF[i] = np.where(
                        self.var.MMF[i] <= (0.4 * self.var.MAF),
                        0.6 * self.var.MMF[i],
                        np.where(
                            self.var.MMF[i] > (0.8 * self.var.MAF),
                            0.3 * self.var.MMF[i],
                            0.45 * self.var.MMF[i],
                        ),
                    )
