    ...
    **Functions**
    ...
    def initial(self):
        ...
        if checkOption('includeWaterBodies') and returnBool('useSmallLakes'):
            ...
            if testRunoff:
                self.var.smalllakeDis0 = loadmap('averageRunoff') *  self.var.smallpart * self.var.cellArea * self.model.InvDtSec
            else:
            ...
            self.var.smalllakeFactor = self.var.smalllakeArea / (self.model.DtSec * np.sqrt(self.var.smalllakeA))
            ...
            smalllakeStorageIndicator = np.maximum(0.0,self.var.smalllakeVolumeM3 / self.model.DtSec + self.var.smalllakeInflowOld / 2)
    ...
    def dynamic(self):
        ...
        **Flow out of lake:**
        ...
        def dynamic_smalllakes(inflow):
            ...
            # here: (Qin1 + Qin2)/2
            self.var.smallLakeIn = lakeIn * self.model.DtSec /self.var.cellArea  # in [m]
            ...
            lakeStorageIndicator = np.maximum(0.0,self.var.smalllakeVolumeM3 / self.model.DtSec  - 0.5 * self.var.smalllakeOutflow + lakeIn)
            ...
            self.var.smalllakeVolumeM3 = (lakeStorageIndicator - self.var.smalllakeOutflow * 0.5) * self.model.DtSec
            # Lake storage

            self.var.smalllakeStorage =  self.var.smalllakeStorage + lakeIn * self.model.DtSec  - QsmallLakeOut - self.var.smallevapWaterBody
