...
class evaporationPot(object):
    ...
    **Functions**
    ...
    def dynamic(self):
        ...
        tasmin_C = self.model.data.to_HRU(data=self.model.data.grid.tasmin, fn=None) - 273.15
        tasmax_C = self.model.data.to_HRU(data=self.model.data.grid.tasmax, fn=None) - 273.15
        ...
        rlus_MJ_m2_day = 4.903E-9 * (((tasmin_C + 273.16) ** 4) + ((tasmax_C + 273.16) ** 4)) / 2  # rlus = Surface Upwelling Longwave Radiation
        ...
        rlds_MJ_m2_day = self.model.data.to_HRU(data=self.model.data.grid.rlds, fn=None) * 0.0864  # 86400 * 1E-6
        ...
        rsds_MJ_m2_day = self.model.data.to_HRU(data=self.model.data.grid.rsds, fn=None) * 0.0864  # 86400 * 1E-6
        # net absorbed radiation of reference vegetation canopy [mm/d]
        RNA = np.maximum((1 - self.var.AlbedoCanopy) * rsds_MJ_m2_day - net_longwave_radation_MJ_m2_day, 0.0)
        # net absorbed radiation of bare soil surface
        RNAWater = np.maximum((1 - self.var.AlbedoWater) * rsds_MJ_m2_day - net_longwave_radation_MJ_m2_day, 0.0)
        # net absorbed radiation of water surface

        vapour_pressure_deficit = np.maximum(saturated_vapour_pressure - actual_vapour_pressure, 0.0)
        slope_of_saturated_vapour_pressure_curve = ((4098.0 * saturated_vapour_pressure) / ((tas_C + 237.3) ** 2))
        ...
        RNANWater = RNAWater / LatHeatVap * slope_of_saturated_vapour_pressure_curve / denominator

        EA = psychrometric_constant * 900 / (tas_C + 273.16) * wind_2m * vapour_pressure_deficit / denominator
