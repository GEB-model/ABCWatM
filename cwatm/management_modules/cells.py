...
class SubVariables(BaseVariables):
    def __init__(self, model):
        self.model = model
        self.scaling = 20

        self.cell_size = self.model.var.cell_size / self.scaling
    ...
    def create_subcell_mask(self):
        with rasterio.open(cbinding('farms'), 'r') as farms_src:
            farms = farms_src.read()[0]
        with rasterio.open(cbinding('land_use_classes'), 'r') as src:
            land_use_classes = src.read()[0]
        return self._create_subcell_mask(farms, land_use_classes, self.model.var.mask, self.scaling)
    ...
    def load_initial(self, *args, **kwargs):
        return self.model.var.load_initial(*args, **kwargs)
    ...
    def load_map(self, fp, fn):
        with rasterio.open(fp) as src:
            array = src.read()[0]
        output = self._load_map(array, self.scaling, self.mixed_size, self.has_subcells.reshape(self.model.var.mask.shape))
        if self.model.conf.use_gpu:
    ...
    def decompress(self, array):
        if isinstance(array, cp.ndarray):
            array = array.get()
        return _decompress_subvar(array, self.subcell_locations, self.scaling, self.model.var.mask)
...
class Cells:
    def __init__(self):
        ...
        self.subvar = SubVariables(self)
        self.subvar.cellArea = self.to_subvar(data=self.var.cellArea, fn='mean')

        # array = self.var.full_compressed(3)
        # array.plot()

        # with rasterio.open('DataDrive/Bhima/land_use/land_use_classes_cwatm.tif') as src:
        #     array = src.read()[0]
        # array = self.subvar.compress(array)
        # array = np.arange(0, self.var.mask_flat.size, dtype=np.float32).reshape(self.var.mask.shape)
        # self.var.plot(array)
        # compressed = self.var.compress(array)
        # self.var.plot(self.var.decompress(compressed))
        # self.var.plot_compressed(compressed)

        # mixed_array = self.to_subcell(compressed)
        # mixed_array = mixed_array + cp.random.uniform(0, 100, size=mixed_array.size)

        # self.subvar.plot(mixed_array)

        # array = self.subvar.load_map('DataDrive/Bhima/agents/farmers/farms.tif', np.mean)
        # self.subvar.plot(array)
        # array = self.to_cell(array)
        # self.var.plot(array)
    ...
    def to_var(self, *, subdata=None, varname=None, fn=None, delete=True):
        ...
        if isinstance(subdata, float):  # check if data is simple float. Otherwise should be numpy array.
            ...
            if checkOption('useGPU') and isinstance(subdata, cp.ndarray):
                subdata = subdata.get()
