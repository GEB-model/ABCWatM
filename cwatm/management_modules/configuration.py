...
class ExtParser(configparser.ConfigParser):
    ...
    def get(self, section, option, raw=False, vars=None, **kwargs):
        ...
        try:
            r_opt = configparser.ConfigParser.get(
            ...
            if sec == "OPTIONS":
                ...
                if opt.lower()[0:4] == "out_":
                    ...
                    if opt.lower()[4:8] == "tss_":
                        outTss[index], check_section = splitout(
                            config.get(sec, opt), check_section
                        )
                    else:
                        outMap[index], check_section = splitout(
                            config.get(sec, opt), check_section
                        )
...
def read_metanetcdf(metaxml, name):
    ...
    if os.path.isfile(metaxml):
        ...
        # using program name
        metaxml = os.path.join(
            os.path.dirname(os.path.realpath(sys.argv[0])), "cwatm", name
        )
        if os.path.isfile(metaxml):
